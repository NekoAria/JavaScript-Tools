...
javascript: void (async () => {
  ...
  try {
    if (["twitter.com", "x.com"].includes(host)) {
      ...
      secondaryUrl = await fetch(
        profileUrl.replace("en/", "").replace("users", "stacc/id")
      ).then((res) => res.url);
      ...
      if (match && match[1]) {
        ...
        throw new Error(
          "Unable to retrieve user information from Bluesky URL."
        );
      }
      ...
      if (scriptTag) {
        ...
        let userId =
          data?.props?.pageProps?.bootstrapEnvelope?.commonBootstrap?.campaign
            ?.data?.relationships?.creator?.data?.id ||
          data?.props?.pageProps?.bootstrapEnvelope?.pageBootstrap?.campaign
            ?.data?.relationships?.creator?.data?.id ||
          data?.props?.pageProps?.bootstrapEnvelope?.pageBootstrap?.pageUser
            ?.data?.id;
        if (userId) {
          profileUrl = location.href
            .replace("http://", "https://")
          ...
          if (
            profileUrl.includes("patreon.com") &&
            !profileUrl.includes("www.patreon.com")
          ) {
        ...
        throw new Error(
          "The necessary data on the Patreon page cannot be found"
        );
      }
      ...
      if (creatorProfileLink) {
        ...
        if (creatorNickname?.getAttribute("value")) {
          secondaryUrl = `https://fantia.jp/${creatorNickname.getAttribute(
            "value"
          )}`;
        } else {
          secondaryUrl = profileUrl;
      ...
      if (host === "xfolio.jp") {
        ...
        const username = document.querySelector(
          "meta[name='misskey:user-username']"
        )?.content;
        if (username) {
          const userId = document.querySelector(
            "meta[name='misskey:user-id']"
          ).content;
